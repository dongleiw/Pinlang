...
VerifyContextResult AstNodeIdentifier::Verify(VerifyContext& ctx) {
	...
	if (v != nullptr) {
		...
		if (vc_param.HasFnCallArgs()) {
			...
			// 这说明该变量是函数
			// 根据参数类型来选择合适的函数重载
			std::string uniq_fnname_suffix = TypeInfoFn::GetUniqFnNameSuffix(vc_param.GetFnCallArgs());
			std::string uniq_fnname		   = m_id + "_" + uniq_fnname_suffix;
			...
			std::string uniq_fnname		  = m_id + "_" + tifn->GetUniqFnNameSuffix();
			Variable*	v				  = ctx.GetCurStack()->GetVariableOrNull(uniq_fnname);
			if (v == nullptr) {
				panicf("var[%s] uniq_fnname[%s] not exist", m_id.c_str(), uniq_fnname.c_str());
			}
